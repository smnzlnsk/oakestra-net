...
func (gw *GatewayConfiguration) configureProxy() error {
	...
	if !ok {
		logger.InfoLogger().Println("Could not find recently added entry by InstanceIP")
	}
	logger.InfoLogger().Println("Successfully added Entry and found by InstanceIP:", entry)

	entry, ok = proxy.Proxy().Environment.GetTableEntryByNsIP(gw.bridgeIPv4)
	if !ok {
		logger.InfoLogger().Println("Could not find recently added entry by NsIP")
	}
